      ...
      {branches.map((branch) => (
        <div key={branch.branch_id} className="rounded-lg bg-white text-center shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 rounded border-gray-300  dark:border-gray-700 ">
          <div className="border-b-2 border-neutral-100 px-6 py-3 dark:border-neutral-600 dark:text-neutral-50">
            ...
            <h2 className="mt-3">Branch{branch.branch_name}</h2>
          </div>
          <div className="border-b-2 border-neutral-100 px-6 py-3 dark:border-neutral-600 dark:text-neutral-50">
          ...
          <div className="border-b-2 border-neutral-100 px-6 py-3 dark:border-neutral-600 dark:text-neutral-50">
            <h2>Branch Code {branch.branch_id}</h2>
          </div>
        </div>
      ))}
    </div>
...
const Branches = () => {
  ...
  useDocumentTitle("User Panel - Branches");
  ...
  useEffect(() => {
      Axios.post("http://localhost:3001/branches").then((response) => {
        let branches: { branch_id: number, branch_name: string, branch_phone: string, branch_address: string, branch_type: string }[] = [];
        response.data.forEach((branchData: { branch_id: number, branch_name: string, branch_phone: string, branch_address: string, branch_type: string }) => {
    ...
    const handleAddBranch = () => {
      if (branchName != "" && branchId != "" && branchPhone != "" && branchAddress != "" && branchType != "Select Agency" && branchType != "") {
        setOpenRegister(false);
        setWrongInfo(false);
        let bType = "";
      if (branchType == "Sales Agency") {
        bType = "sales";
      } else if (branchType == "Damage Agency") {
        bType = "damage";
      } else {
        bType = "sales,damage";
      }
  ...
  return (
    <div>
      ...
      <h1 className="branch-title">Register New Insurance Branch</h1>
        <DialogContent>
          <div className="md:flex items-center">
            <div className="md:w-72 flex flex-col contact-item">
            <label className="text-end leading-none text-gray-800">Insurance Code</label>
              <input
                onChange={(e) => {
            ...
            <label className="text-end leading-none text-gray-800">Insurance Name</label>
              <input
                onChange={(e) => {
                ...
                type="name"
                className="text-end leading-none text-gray-900 p-3 mt-4 bg-gray-100 placeholder-gray-300"
                placeholder="Independent Insurance"
              />
            </div>
          </div>
          <div className="md:flex items-center mt-8">
            <div className="md:w-72 flex flex-col">
            <label className="text-end leading-none text-gray-800">Insurance Contact Number</label>
              <input
                onChange={(e) => {
                ...
                type="name"
                className="text-end leading-none text-gray-900 p-3 mt-4 bg-gray-100 placeholder-gray-300 "
                placeholder="+213454567"
              />
            </div>
            <div className="md:w-72 flex flex-col md:ml-6 md:mt-0 mt-4">
              <label className="text-end leading-none text-gray-800">Insurance Address</label>
              <input
                onChange={(e) => {
          ...
          <div className="mt-10" style={{ textAlign: "end" }}>
            ...
            >
              <option style={{ textAlign: "end" }} selected>Choose an agency
              </option>
              <option style={{ textAlign: "end" }}>Sales Agency</option>
              <option style={{ textAlign: "end" }}>Damage Agency</option>
              <option style={{ textAlign: "end" }}>Sales and Damage Agency</option>
            </select>
          </div>
          {wrongInfo ? (
            <div className="wrong-info mt-10">
              <p>Enter complete and accurate information</p>
      ...
      <h1 className="branch-title">Edit Branch</h1>

        <DialogContent>
          <div className="md:flex items-center">
            <div className="md:w-72 flex flex-col contact-item">
              <label className="text-end leading-none text-gray-800">code bima</label>
              <input
                onChange={(e) => {
                  setBranchId(e.target.value);
                }}
            ...
            <label className="text-end leading-none text-gray-800">Insurance name</label>
              <input
                ...
                type="name"
                className="text-end leading-none text-gray-900 p-3 mt-4 bg-gray-100 placeholder-gray-300"
                placeholder="Insurance for veterans"
              />
            </div>
          </div>
          <div className="md:flex items-center mt-8">
            ...
            <div className="md:w-72 flex flex-col md:ml-6 md:mt-0 mt-4">
            <label className="text-end leading-none text-gray-800">Adress</label>
              <input
                onChange={(e) => {
                  setBranchAddress(e.target.value);
                }}
          ...
          <div className="mt-10" style={{ textAlign: "end" }}>
            ...
            >
              ...
              <option style={{ textAlign: "end" }}>Sales dealership</option>
              <option style={{ textAlign: "end" }}>Claim dealership</option>
              <option style={{ textAlign: "end" }}>Sales and claim dealership</option>
            </select>
          </div>
          {wrongInfo ? (
            <div className="wrong-info mt-10">
              <p>Enter the details completely and correctly</p>
            </div>
          ) : null}
        </DialogContent>
        <div style={{ margin: "auto" }}>
      ...
      <Dialog fullScreen={fullScreen} open={openDelete} onClose={handleDeleteClickClose} aria-labelledby="responsive-dialog-title">
        <h1 className="branch-title">Delete Branch</h1>
        <DialogContent>
          <div className="md:flex items-center">
            <div className="md:w-72 flex flex-col contact-item">
              <label className="text-end leading-none text-gray-800">insurance</label>
              <input
                onChange={(e) => {
                  setBranchId(e.target.value);
                }}
